...
class UserPasswordPage extends Component {
  ...
  componentWillReceiveProps = (nextProps) => {
    ...
    if (typeof error !== typeof 'string' || error.length > 0) Alert.alert(Translate.t('user.password.alertTitle'), Translate.t('user.password.alertWrongPassword'));
  }

  handleUpdatePassword = async () => {
    const { actual, next, nextRepeat } = this.state;
    const {
      dispatchUpdatePassword, navigation, role, email,
    } = this.props;
    if (next !== nextRepeat) Alert.alert(Translate.t('user.password.alertTitle'), Translate.t('user.password.alertSamePassword'));
    else if (!Helper.isValidPassword(next, nextRepeat)) Alert.alert(Translate.t('user.password.alertTitle'), Translate.t('user.password.alertShortPassword'));
    else {
