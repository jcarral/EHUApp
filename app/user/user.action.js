...
export const addNewSub = (type, data) => async dispatch => {
	try{
		dispatch({
			type: START_NEW_SUBSCRIPTION,
		});
		await addSubscriptionOnFirebase(type, data);
		let type;
...
export const deleteSubscription = (type, key) => async dispatch => {
	try{
		dispatch({
			type: START_DELETING_SUBSCRIPTION,
		});
		await deleteSubscriptionOnFirebase(type, key);
		let type;
		if (type === 'subjects') type = SUCCESS_NEW_SUBSCRIPTION_SUBJECTS;
		else if (type === 'teachers') type = SUCCESS_NEW_SUBSCRIPTION_TEACHERS;
		else return dispatch({
